...
from internvl.model.internvl_chat import (InternVisionConfig,
                                          ...
                                          InternVLChatConfig,
                                          InternVLChatModel)
from internvl.model.internvl_seg import InternVLMagnifier
...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    preprocess_phi3, preprocess_internvl2_5_mag,
...
class ModelArguments:
    ...
    #---seg_model & mformer params
    seg_model: str = field(
        default=None,
        metadata={'help': 'The checkpoint to use for the seg model.'}
    )
    segmentation_model_path: str = field(
        default="/cpfs01/shared/ADLab/hug_ckpts/chenxiangnan/sam_vit_h_4b8939.pth",
        metadata={'help': 'The path to the segmentation model.'}
    )
    freeze_seg: bool = field(
        default=False,
        metadata={'help': 'Set to True to freeze the MLP. Default is False.'},
    )
    mixed_strategy: str = field(
        default=None,
        metadata={'help': 'The mixed strategy for mix visual tokens. Default is None.'}
    )
    ...
    seg_tokens: int = field(
        default=64,
        metadata={'help': 'The required token length of final visual tokens. Default is 64.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        #--mag 参数
        self.model = model
        self.transform_sam = ResizeLongestSide(1024)
        self.transform = build_transform(is_train=False, input_size=image_size)
        self.sam_size = 1024
        self.ignore_label = 255
        self.seg_tokenizer = self.model.seg_tokenizer
    ...
    def get_preprocess_function(self):
        ...
        elif self.template_name == 'internvl2_5_mag':
            preprocess_function = preprocess_internvl2_5_mag
        else:
            ...
            try:
                ...
                if 'image' in data_item and len(data_item['image']) != 0:
                    if type(data_item['image']) == list:
                        ...
                        ret = self.mag_get_item(data_item)
...
def freeze_modules(model, model_args):
    ...
    if model_args.freeze_seg:
        _freeze_params(model.seg_model)
    ...
    if model_args.freeze_mlp:
        _freeze_params(model.mlp_clip)
        _freeze_params(model.mlp_seg)
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = InternVLMagnifier.from_pretrained(
            model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config, llm_tokenizer=tokenizer, args=model_args)
    else:
    ...
    #----初始化seg model 组件
    model.init_seg_mformer(model_args)
    logger.info("SEG Modules are built!")
    ...
    train_dataset = build_datasets(
        ...
        max_num_frame=data_args.max_num_frame, seg_tokens=model.config.seg_tokens,
        mixed_strategy=model_args.mixed_strategy)
    a = train_dataset[0]
    #----冻结部分参数
    model = freeze_modules(model, model_args)
    ...
    if data_args.use_packed_ds:
        ...
        collator = concat_mag_data_collator
