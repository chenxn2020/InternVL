...
def dice_loss(
    ...
    #Dice Loss 是一种基于 Dice 系数的损失函数，用于衡量预测结果和真实标签的相似性。
    #二者重叠区域的像素数的比例
    inputs = inputs.sigmoid()
    inputs = inputs.flatten(1, 2)
    targets = targets.flatten(1, 2)
    numerator = 2 * (inputs / scale * targets).sum(-1) #重叠的mask像素数
    denominator = (inputs / scale).sum(-1) + (targets / scale).sum(-1) #mask像素总数
